...
验证"持续访问新栈页会持续缺页"这一关键观察，对比两种栈使用模式：
1. 固定深度重复调用（页表复用）
2. 持续增长递归深度（持续缺页）
...
| 场景 3（重复递归） | ~200 次（首次） | 0.056 ms | 112 ns/层 | - |

### 验证结论

✅ **持续访问新栈页会持续缺页**
- 场景 2：424 次缺页，接近理论值 400 次
- 每层递归访问新的虚拟页，每次都触发缺页

✅ **重复访问已访问页几乎不缺页**
- 场景 1：100 次重复调用，仅首次 ~4 次缺页
- 页表映射保留，后续 99 次调用 0 次缺页

✅ **性能差异显著**
- 持续缺页（场景 2）vs 页表复用（场景 1）：**23 倍**
- 单次缺页成本：(2715 - 118) ns ≈ **2.6 μs**

✅ **栈的优势是"历史访问缓存"**
- 不是"栈永远不缺页"
- 而是"页表持久性"——重复访问的区域不缺页
- 实际应用多是相同深度重复调用，因此栈表现"快"
...
# 错误（优化会消除栈分配）
gcc -O2 -Wall -Wextra -g -o stack_growth_comparison src/stack_growth_comparison.c
```
